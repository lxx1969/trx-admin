...
router.post('/cellphone', async (req, res) => {
  try {
    ...
    if (!data[0]) {
      const insertSql = `INSERT INTO tb_user(username,password,phone) VALUES(${phone},'123456',${phone});`
      const select = `SELECT id,username FROM tb_user where phone='${phone}' into @id,@username;`
      const insertUserInfo = `INSERT INTO tb_userinfo(id,username) VALUES(@id,@username)`;
      await mysql.query(insertSql);
      await mysql.query(select);
      await mysql.query(insertUserInfo);
...
router.post('/editInfo', async (req, res) => {
  try {
    ...
    var sql = `update tb_userinfo set username='${username}',position='${position}',location='${location}',label='${label}',skill='${skill}' where id='${temp.id}'`
    var update = `update tb_user set username='${username}' where id='${temp.id}'`;
    await mysql.query(sql);
    await mysql.query(update)
